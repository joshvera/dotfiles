# `zsh -ic '...'` is interactive, so it reads `.zshrc`, but shells launched
# through pipes or other non-TTY entry points do not have a real line editor or
# prompt surface. Powerlevel10k, gitstatus, `zle`, `bindkey`, and some plugin
# startup paths assume a TTY and otherwise emit noisy errors such as
# `can't change option: zle` or `gitstatus failed to initialize`.
#
# Skip the TTY-bound UI layer in those shells and let the lightweight
# environment / PATH setup later in this file continue to run.
if [[ -t 0 && -t 1 ]]; then
...
# Keep local developer overrides ahead of concrete tool installs added during
# interactive shell startup and prompt-time environment refreshes. In
# particular, `mise activate zsh` installs a `precmd` hook that can prepend
# versioned tool bins again on every prompt, so a one-time reorder at the end
# of `.zshrc` is not stable.
_vera_local_path_overrides() {
...
_vera_local_path_overrides
autoload -U add-zsh-hook
add-zsh-hook precmd _vera_local_path_overrides
