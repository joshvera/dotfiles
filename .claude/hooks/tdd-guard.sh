...
if [[ "$FILE_PATH" == *".swift" ]]; then
    ...
    REPORTER="/Users/vera/github/tdd-guard/reporters/swift/.build/debug/tdd-guard-swift"
    if [ -x "$REPORTER" ]; then
    ...
    REPORTER="/Users/vera/github/tdd-guard/reporters/rust/target/debug/tdd-guard-rust"
    if [ -x "$REPORTER" ]; then
        "$REPORTER" --project-dir "$PROJECT_DIR" >/dev/null 2>&1
        # Check if markers were found by examining the report
